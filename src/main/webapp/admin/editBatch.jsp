...
<div id="main-content">
    <div class="main-container">
        ...
        <form action="<%=request.getContextPath()%>/admin/editBatch" method="post">
            <input type="hidden" name="batchID" value="${batch.batchID}"/>
            <div class="mb-3">
                <label for="productID" class="form-label">Sản phẩm</label>
                <select class="form-select" id="productID" name="productID" required>
                    <c:forEach items="${products}" var="product">
                        <option value="${product.productID}" ${product.productID == batch.productID ? 'selected' : ''}>${product.productName}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="mb-3">
                <label for="batchNumber" class="form-label">Mã lô</label>
                <input type="text" class="form-control" id="batchNumber" name="batchNumber" value="${batch.batchNumber}" required/>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="importDate" name="importDate" value="${batch.importDate}" required/>
            </div>
            <div class="mb-3">
                <label for="expiryDate" class="form-label">Hạn sử dụng</label>
                <input type="date" class="form-control" id="expiryDate" name="expiryDate" value="${batch.expiryDate}"/>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="supplierID" name="supplierID">
                    ...
                    <c:forEach items="${suppliers}" var="supplier">
                        <option value="${supplier.supplierID}" ${supplier.supplierID == batch.supplierID ? 'selected' : ''}>${supplier.supplierName}</option>
                    </c:forEach>
                </select>
            </div>
